...
def test_question_database_index1(database):
    ...
    assert (
        sorted(list(set(database.questions_base.index.get_level_values(0))))
        == exp_database_index1
    )
...
def test_question_database_index2(database):
    ...
    assert (
        sorted(list(set(database.questions_base.index.get_level_values(1))))
        == exp_database_index2
    )
...
def test_add_question(database):
    database.add_question_to_base(
        22, "Historia Polski", "question", "A", "B", "C", "D", "A"
    )
    with open(database.base_file_path) as file:
        sum_lines = sum(1 for line in file)
        expected_sum = 23
    assert sum_lines == expected_sum
